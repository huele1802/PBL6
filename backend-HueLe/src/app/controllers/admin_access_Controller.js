...
class admin_access_Controller {
    add_Admin = async (req, res) => {
        try {
            ...
            const admin = await Admin_Access.create({
                user_id,
                read_access,
                write_access,
                admin_write_access,
            ...
            const new_Admin = await Admin_Access.findById(admin._id).populate(
                "user_id",
    ...
    remove_Admin = async (req, res) => {
        try {
            ...
            // if no ids
            if (!access_Ids || !Array.isArray(access_Ids) || access_Ids.length === 0) {
    ...
    admin_List = async (req, res) => {
        try {
            const admin_Access = await Admin_Access.find({}).populate(
                "user_id",
                "email username profile_image"
            )
    ...
    admin_Detail = async (req, res) => {
        try {
            ...
            const admin_Access = await Admin_Access.findOne({ user_id: access_id }).populate(
                "user_id",
                "email username profile_image"
            )
